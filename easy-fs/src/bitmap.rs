...
impl Bitmap {
    ...
    /// For every block (512 bytes) in `start_block_id`..`start_block_id`+`blocks`,
    /// enumerate every 64 bits to find a group that is not full.
    /// Then alloc a bit in it and return that position.
    ...
    pub fn dealloc(&self, block_device: &Arc<dyn BlockDevice>, bit: usize) {
        ...
        get_block_cache(self.start_block_id + block_pos, Arc::clone(block_device))
            .lock()
            .modify(0, |bitmap_block: &mut BitmapBlock| {
