...
use crate::{drivers::net::NET_DEVICE, fs::File};

use super::{
    net_interrupt_handle,
    socket::{add_socket, get_seq_ack_by_index, pop_data, remove_socket},
    LOSE_NET_STACK,
};
...
impl File for TCP {
    ...
    fn write(&self, buf: crate::mm::UserBuffer) -> usize {
        ...
        let len = data.len();
        let (ack, seq) = get_seq_ack_by_index(self.socket_index).map_or((0, 0), |x| x);
